...
void my_exit_group(int status)
{
	...
	//del pid from all list of monitoring pids.
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	int sysc = reg.ax; //system call number
	asmlinkage long (*orig_call)(struct pt_regs);

	spin_lock(&sys_call_table_lock);
	spin_lock(&my_table_lock);
	//check if intercepted, then do the interceptoion procedure.
	...
	//store original call in a variable	
	orig_call = table[sysc].f;
	//unlock spins before calling original func.
	spin_unlock(&sys_call_table_lock);
	spin_unlock(&my_table_lock);
	//call the original call
	return orig_call(reg);
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	spin_lock(&sys_call_table_lock);
	//error checking.
	check = error_check(cmd, syscall, pid);
	if(check != 0){
		spin_unlock(&my_table_lock);
		spin_unlock(&sys_call_table_lock);
		return check;
	...
	//intercept the syscall
		...
		//set the syscall intercepted to 1.
		table[syscall].intercepted = 1;

		//set the original function in table to the syscall function in syscall table
		table[syscall].f = sys_call_table[syscall];

		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = &interceptor; //pointer to interceptor
		...
		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = table[syscall].f; //the original syscall thats stored in the table
		set_addr_ro((unsigned long) sys_call_table);
		//destroy the list since no need of it anymore
		...
		//check pid permissions
			...
			//destroy list otherwise, since pid==0
			else{
				destroy_list(syscall);
		...
		//delete from monitoring list.
		if(pid == 0){

			//delete the syscall's entire pid list
			destroy_list(syscall);
...
//check errors for my_syscall and return err code as necessarry
int error_check(int cmd, int syscall, int pid){
	//check syscall is valid
	if(syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL)
		return -EINVAL;
	//chek pid is non negative
	if(pid < 0)
		return -EINVAL;
	//check if pid is a valid pid (the pid thats > 0)
	if(pid !=0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)
		return -EINVAL;

	//if not root user for the first two commands
	if((cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) && current_uid() != 0)
		return -EPERM;

	//root user checker for last two commands
	...
	//cannot deintercept a call thats not intercepted yet
	if( cmd == REQUEST_SYSCALL_RELEASE){
		if(table[syscall].intercepted == 0)
			return -EINVAL;
	}

	//can't stop monitoring if not already being monitored.
	if(cmd == REQUEST_STOP_MONITORING){
		if(table[syscall].monitored == 0 || table[syscall].intercepted == 0)
			return -EINVAL;
	}

	//can't intercept if already intercepted
	if(cmd == REQUEST_SYSCALL_INTERCEPT){
		if(table[syscall].intercepted == 1)
			return -EBUSY;
	}

	//check if not already being monitored
...
static int init_function(void) {
	...
	// spin_unlock_init(&my_table_lock);
	// spin_unlock_init(&sys_call_table_lock);
	return 0;
}
